...
%% Alokacja wektor闚
% Trajektora zadana na ca造m horyzoncie
...
% Przyrosty sterowan
detu = zeros(2*Hs,1);
% Niemierzealne zak堯cenia i niedok豉dnoi modelu
d_h = zeros(kend,1);
d_pH = zeros(kend,1);
% Sterowania modelu
u = zeros(2,kend);
...
e1=(y_zad_h - yp_h)'*(y_zad_h - yp_h);
e2=(y_zad_pH - yp_pH)'*(y_zad_pH - yp_pH);
...
ylabel('h_{zad}, h');
xlabel('Krok [k]');
title(['Regulacja GPC [h], MSE_h=',num2str(e1)]);
legend('h_{zad}','h')
...
ylabel('pH_{zad}, pH');
xlabel('Krok [k]')
title(['Regulacja GPC [pH], MSE_{pH}=',num2str(e2)]);
legend('pH_{zad}','pH','Location','southeast');
